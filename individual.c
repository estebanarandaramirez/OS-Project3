...
pthread_mutex_t print_lock = PTHREAD_MUTEX_INITIALIZER; // lock for altering bath
double grossRevenue = 0;
double waitTime;

int Individual(struct thread_args *args);
double calc_avg(double last_ave, int count, double new_data);
void mysleep(useconds_t sleep_time);
...
int main(int argc, char *argv[]) {
  ...
  pthread_t threads[numUsers]; // threads struct
  int count = 0;
  ...
  for(int i = 0; i < pirates; i++){
    ...
    while(willReturn == true && visits <= MAX_VISITS){
      visits++;
    ...
    pthread_create(&threads[count], NULL, (void *) &Individual, (void *) args[count]);
    count++;
  }
  ...
  Finalize(grossRevenue); // close hte bathroom
  return 0;
}

int Individual(struct thread_args *args){
  useconds_t time_to_arrive, time_to_costume;
  struct custStats cs;
  int visitCount = args->numVisits;
  for(int i=0; i<visitCount;i++){ // iterate over number of times a thread wishes to use the bathroom
    time_to_arrive = (useconds_t)(randomizeTime(args->meanCostuming)); // gen random number for time to wait using mean_arrtime
    mysleep(time_to_arrive); // wait until needing to use the bathroom
    Enter(args->c); // attempt to enter the bathroom
    cs.waitTime[i] = waitTime;
    time_to_costume = (useconds_t)(randomizeTime(args->meanCostuming)); // when in the bathroom, generate random time to wait
    ...
    mysleep(time_to_costume); // stay in the bathroom
    Leave(); // leave bathroom
  }
  // print thread info
...
void printCustomer(enum customer c){ // Prints the gender g
  if (c == pirate){
    printf("Pirate\n");
  }
  else if (c == ninja){
...
void mysleep(useconds_t sleep_time){ // Sleeps for sleep_time number of microseconds
...
double getmytime() { // Get time in ms in double format
  struct timeval tim;
  gettimeofday(&tim,0);
